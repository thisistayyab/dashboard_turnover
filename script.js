// Standard page with no manual add
// Supabase integration (optional): when window.sb is available, we sync data with table `consultants`.
// LocalStorage remains the source of truth if Supabase is not configured or offline.
const STORAGE_KEY = "turnover_standard_v1";
...
function saveData(d) {
  ...
  // Best-effort cloud sync (debounced)
  if (isSupabaseReady()) {
    queueCloudSync(d);
  }
}
...
async function supaSyncAll(list){
  ...
  // Simple strategy: replace all rows for this dataset by truncating then inserting
  // This avoids needing stable IDs in the current UI model.
...
document.addEventListener("DOMContentLoaded", async () => {
  // If Supabase becomes ready, hydrate from cloud once
